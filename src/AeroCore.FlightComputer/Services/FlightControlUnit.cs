...
namespace AeroCore.FlightComputer.Services
{
    ...
    public class FlightControlUnit : IFlightComputer
    {
        ...
        private readonly Channel<ControlCommand> _commandQueue = Channel.CreateBounded<ControlCommand>(new BoundedChannelOptions(100)
        {
        ...
        private async Task ProcessCommandsAsync(CancellationToken ct)
        {
            ...
            try
            {
                // Optimization: Use Reader.ReadAllAsync to process commands immediately without polling latency.
